Ext.define(MyIndo.getNameSpace('controller.Kredit.AnalisaKredit'), {
	extend: 'MyIndo.app.Controller',

	requires: [
	MyIndo.getNameSpace('view.Kredit.AnalisaKredit.Detail')
	],

	init: function() {
		this.control({
			'analisakreditview button': {
				click: this.onButtonClicked
			},
			'detailpermohonankreditview button': {
				click: this.onButtonClicked
			}
		});
	},
	...
	onButtonClicked: function(record) {
		...
		switch(action) {
			case 'detail':
				...
				break;
			/* Detail Window */
			case 'analisakredit-accept':
				this.accept(record);
				break;
			case 'analisakredit-decline':
				this.decline(record);
				break;
			case 'analisakredit-cancel':
				record.up().up().close();
				break;
		}
	},
	...
	detail: function(record) {
		...
		if(selected.length > 0) {
			var popWindow = Ext.create(MyIndo.getNameSpace('view.Kredit.AnalisaKredit.Detail'));
			var form = popWindow.items.get('analisakredit-add-update-form').getForm();
			form.setValues({
	...
	accept: function(record) {
		var form = Ext.getCmp('analisakredit-add-update-form').getForm();
		var values = form.getValues();
		var me = this;
		Ext.Msg.confirm('Permohonan Kredit', 'Anda yakin ingin menyetujui Permohonan Kredit ini ?', function(btn) {
			if(btn == 'yes') {
				Ext.Ajax.request({
					...
					success: function(data) {
						...
						Ext.Msg.alert('Permohonan Kredit', 'Permohonan Kredit berhasil Disetujui.');
	...
	decline: function(record) {
		var form = Ext.getCmp('analisakredit-add-update-form').getForm();
		var values = form.getValues();
		var me = this;
		Ext.Msg.confirm('Permohonan Kredit', 'Anda yakin ingin menolak Permohonan Kredit ini ?', function(btn) {
			if(btn == 'yes') {
				Ext.Ajax.request({
					url: MyIndo.siteUrl('permohonankredit/request/status'),
					params: {
					...
					success: function(data) {
						...
						Ext.Msg.alert('Permohonan Kredit', 'Permohonan Kredit berhasil ditolak.');
						var mainContent = Ext.getCmp('main-content');
						var store = mainContent.getActiveTab().getStore();
						store.load();
